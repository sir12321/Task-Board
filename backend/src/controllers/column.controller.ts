...
export const editColumn = async (
  ...
  try {
    const userId = req.user?.id;
    const globalRole = req.user?.globalRole;
    const columnId = Array.isArray(req.params.id)
      ? req.params.id[0]
      : req.params.id;
    ...
    if (err instanceof Error && err.message.includes('Forbidden')) {
      ...
      res
        .status(400)
        .json({
          error: err instanceof Error ? err.message : 'Failed to update column',
        });
    }
  }
};
  ...
  try {
    ...
    if (!userId || !columnId || !['left', 'right'].includes(direction)) {
      res
        .status(400)
        .json({
          error: 'Invalid request: direction must be "left" or "right"',
        });
      return;
    }
