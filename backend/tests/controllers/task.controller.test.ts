...
describe('Task Controller', () => {
    ...
    describe('POST /api/tasks', () => {
        it('creates a task and returns 201', async () => {
            ...
            expect(auditService.logAct).toHaveBeenCalledWith('task-1', 'user-1', 'CREATED');
        });
    ...
    describe('DELETE /api/tasks/:id', () => {
        ...
        it('returns 400 when the task still has open subtasks', async () => {
            vi.mocked(taskService.removeTask).mockRejectedValue(
                new Error('Cannot delete: task has subtasks that must be removed first'),
            );
    ...
    describe('GET /api/tasks/:id', () => {
        ...
        it('returns 404 when the task does not exist', async () => {
            vi.mocked(taskService.getData).mockResolvedValue(null);

            const res = await request(buildApp('user-1')).get('/api/tasks/ghost-task');
